      ...
      {tournaments.length === 0 ? (
        ...
        >
          {tournaments.map((t) => {
            ...
            return (
              <li key={t.id} className="panel" style={{ padding: 20 }}>
                <div
                  aria-hidden
                  style={{ height: 3, width: 52, background: t.accent_color, marginBlockEnd: 14 }}
                />
                <Link href={`/tournaments/${t.slug}`}>
                  <h2 style={{ fontSize: 19, marginBlockEnd: 6 }}>{t.name}</h2>
                </Link>
                {t.description ? (
                  <p
                    style={{
